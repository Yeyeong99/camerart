...
function clickCalculator() {
    if (click == 0) {
        ...
        changeSection("드디어 점심을 먹는 당신."+"<br>"+"심심해서 동영상을 볼까 해요.", "영상을 틀었는데.."+"<br>"+"저화질로 나오네요?", "최고화질 아니면.. 왜 봐?"+"<br>"+"무조건 최고화질로 본다", "그냥 틀어지는 대로 본다", "안볼래. 그냥 화면을 끈다.")
        body.style.backgroundImage = "";
        body.style.backgroundSize = "";
        optionBtn[1].classList.remove("hide");
        click += 1;
    } else if (click == 5) {
        changeSection("바깥 공기를 쐬고 싶어졌다." +"<br>"+"집 앞 카페에 왔어요.", "음료를 시켰는데..", "마시면서 집에 가야지!"+"<br>"+"일회용 테이크 아웃잔에 담아갈게요~", "카페에서 일회용 컵으로.."+"<br>"+"하지만 빨대랑 컵홀더는 안주셔도 돼요!", "챙겨온 텀블러에 담아갈게요~") 
        click += 1;
    } else if (click == 6) {
        changeSection("집에 돌아왔네요.", "샤워를 하려고 해요.", "느긋하게 씻어야죠"+"<br>"+"(15분 초과)", "호다닥 씻을래요"+"<br>"+"(10분 내외)", "땀도 안흘렸는데…"+"<br>"+"양치랑 세수만")
        click += 1;
    } else if (click == 7) {
        changeSection("세탁기 앞에 선 당신.", "세탁기 물의 온도는?", "그냥 세탁기가 맞춰놓은 대로", "옵션2", "내가 설정해야지! 별도로 낮춘다")
        optionBtn[1].classList.add("hide");
        click += 1;
    } else if (click == 8) {
        changeSection("띠링- 새로운 메일이 왔네요!", "당신의 메일함은", "메일…그런 거.. 왜 읽어?"+"<br>"+"안 건드려서 미확인 메일이 999+", "남은 메일 100개 정도."+"<br>"+"미확인 메일은 없다", "확인하고 바로 지우는 타입."+"<br>"+"메일함이 깨끗하다")
        optionBtn[1].classList.remove("hide");
        click += 1;
    } else if (click == 9) {
    ...
    optionBtn[i].addEventListener("click", function (e) {
        ...
        if (click !== 3) {
            ...
            if(e.target.innerHTML === "한국인은 역시 배달이지…"+"<br>"+"시켜먹는다.") {
                ...
                changeSection("집에서 요리를 하는 당신.", "양은 얼마나 할까요?", "자고로 부족한 것 보다는"+"<br>"+"많은 게 낫지. 많이 한다!", "옵션2", "음식물 쓰레기"+"<br>"+"버리면 안돼! 적게 한다.")
                optionBtn[1].classList.add("hide");
